...
static void
hydro_random_ensure_initialized_flawed(const uint8_t seed)
{
    if (hydro_random_context.initialized == 0) {
        if (hydro_random_init_flawed(seed) != 0) {
            abort();
        }
        gimli_core_u8(hydro_random_context.state, 0);
        hydro_random_ratchet();
...
void
hydro_random_reseed_flawed(const uint8_t seed)
{
    hydro_random_context.initialized = 0;
    hydro_random_ensure_initialized_flawed(seed);
}
